...
@Component({
  selector: 'app-arboldeobjetivos3pdf',
  templateUrl: './arboldeobjetivos3pdf.component.html',
  styleUrls: ['./arboldeobjetivos3pdf.component.css']
})
export class Arboldeobjetivos3pdfComponent {
  ...
  actividad8:any;
  elcorreo:any;
  //variables para poder crear el pdf
  Fin:any;
  Proposito:any;
  Componente1:any;
  Componente2:any;
  ...
  regresarUnpoco(): void {
    //ruta para nevegacion
    this.router.navigate(['home/login/proyectos/'+this.elcorreo+'/arboldeobjetivos3/'+this.elproyecto+'/arboldeobjetivos3creado/'+3]);
  }
  ...
  downloadPDF() {
    ...
    html2canvas(INFORMACION, options).then((canvas) => {
      ...
      //aqui le controlo para que el imprimir se vaya la descarga con la fecha del sistema y el nombre
      docResult.save(`${new Date().toISOString()}arbol-objetivos-diseño3.pdf`);
    });
  }
