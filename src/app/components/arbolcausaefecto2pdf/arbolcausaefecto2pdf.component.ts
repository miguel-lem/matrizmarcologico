...
@Component({
  selector: 'app-arbolcausaefecto2pdf',
  templateUrl: './arbolcausaefecto2pdf.component.html',
  styleUrls: ['./arbolcausaefecto2pdf.component.css']
})
export class Arbolcausaefecto2pdfComponent {
  ...
  //variables para control de la extraccion
  Efecto_directo:any;
  Problema:any;
  Causa_directa1:any;
  Causa_directa2:any;
  ...
  regresarProyectos(): void {
    this.router.navigate(['home/login/proyectos/'+this.elcorreo+'/arbolcausaefecto2/'+this.elproyecto]);
  }
  ...
  downloadPDF() {
    ...
    html2canvas(INFORMACION, options).then((canvas) => {
      ...
      docResult.save(`${new Date().toISOString()}arbol-causa-efecto2.pdf`);
    });
  }
